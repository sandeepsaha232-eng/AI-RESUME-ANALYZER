...
export default function MarketingLanding({ onGetStarted, onLogin, onInstantResumeParsed, reduceMotion = false }: MarketingLandingProps) {
  ...
  return (
    <div className="relative min-h-screen flex flex-col justify-between overflow-x-hidden bg-[#0a0a0c] text-slate-200 transition-colors duration-300 font-sans">

      {/* Subtle Aesthetic Stars/Mesh (No Neon Glowing Blobs) */}
      <CanvasVisualizer intensity="low" reduceMotion={reduceMotion} />

      {/* Header (Professional, Aesthetic Matte Finish) */}
      <header className="sticky top-0 z-50 w-full bg-[#0a0a0c]/80 backdrop-blur-md border-b border-white/5 h-16 flex items-center justify-between px-4 sm:px-8">
        <div className="flex items-center space-x-2 cursor-pointer">
          <div className="p-1.5 bg-gradient-to-br from-slate-700 to-slate-500 rounded-lg text-white shrink-0">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[8px] font-semibold text-slate-400 uppercase tracking-widest">Premium Edition</span>
          </div>
        </div>
        <div className="flex items-center space-x-4">
          ...
          <button onClick={onGetStarted} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white font-semibold text-[11px] sm:text-xs rounded-lg transition-all uppercase tracking-wider border border-white/5">Build Resume</button>
        </div>
      </header>
      ...
      <main className="flex-grow space-y-16 sm:space-y-24 md:space-y-32 py-12 px-4 sm:px-8 max-w-7xl mx-auto w-full relative z-10 text-center">
        ...
        <section className="space-y-6 max-w-3xl mx-auto pt-2">
          <div className="inline-flex items-center space-x-2 bg-slate-900/60 text-slate-300 px-3.5 py-1.5 rounded-full border border-slate-850 text-[9px] sm:text-[10px] font-semibold uppercase tracking-widest">
          ...
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white leading-tight tracking-tight">
            ...
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-slate-100 via-slate-300 to-amber-200">
              Professional Grade.
            </span>
          </h1>
          ...
          <p className="text-sm sm:text-lg text-slate-300 font-light max-w-2xl mx-auto leading-relaxed hidden sm:block">
            Stop pasting raw prompts into ChatGPT. Elevate combines <span className="text-white font-medium">deterministic recruiter rules</span> and custom-engineered Gemini optimization to audit, rate, and tailor your resume flawlessly.
          </p>

          <div className="pt-3 flex flex-col sm:flex-row justify-center items-center gap-3 max-w-xs sm:max-w-md mx-auto w-full">
            <button onClick={onGetStarted} className="w-full py-3.5 bg-white hover:bg-slate-100 text-slate-950 font-bold text-[11px] sm:text-xs rounded-lg shadow-sm uppercase tracking-wider flex items-center justify-center space-x-2">
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center max-w-4xl mx-auto">
          <div className="p-4 bg-white/[0.02] border border-white/5 backdrop-blur-sm rounded-xl">
          ...
          <div className="p-4 bg-slate-900/[0.4] border border-slate-800 rounded-xl">
            <p className="text-xl sm:text-2xl font-bold text-white">{counter}</p>
        ...
        <div id="dropzone-element" className="w-full max-w-xl mx-auto space-y-4">
          <AnimatePresence mode="wait">
            {!analysisResult ? (
              <motion.div
                key="dropzone"
                className="relative rounded-2xl p-[1px] bg-white/5 hover:bg-white/10 transition-all duration-300"
              >
                <div
                  ...
                  onDrop={handleDrop}
                  className={`relative rounded-[15px] bg-[#0d0d10] p-6 sm:p-10 text-center transition-all ${
                    dragActive ? 'bg-slate-900' : ''
                  }`}
                >
                  <input
                    type="file"
                  ...
                  {isAnalyzing ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <label htmlFor="frictionless-file-upload" className="inline-flex items-center space-x-1.5 px-4 py-2 bg-slate-800 hover:bg-slate-750 text-white text-xs font-semibold rounded-lg cursor-pointer transition-colors">
              ...
              <motion.div
                key="rating-card"
                className="relative rounded-2xl p-[1px] bg-slate-800 shadow-xl"
              >
                <div className="rounded-[15px] bg-[#0c0c10] p-6 sm:p-8 text-center space-y-4 relative overflow-hidden">
                  ...
                  <div className="flex justify-center">
                    <div className="relative w-20 h-20 sm:w-24 sm:h-24">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle cx="50" cy="50" r="40" fill="transparent" strokeDasharray={2 * Math.PI * 40} strokeDashoffset={2 * Math.PI * 40 * (1 - analysisResult.score / 100)} strokeWidth="7" strokeLinecap="round" className="stroke-slate-400" />
                      </svg>
                  ...
                  <div className="max-w-sm mx-auto p-3 bg-white/[0.02] border border-white/5 rounded-lg">
                    <p className="text-xs text-slate-300 leading-relaxed font-light">"{analysisResult.pickupLine}"</p>
                  </div>
                  <button onClick={claimWorkspace} className="w-full sm:w-auto px-5 py-2.5 bg-white hover:bg-slate-100 text-slate-950 font-bold text-xs rounded-lg transition-colors flex items-center justify-center space-x-1.5 mx-auto uppercase tracking-wide">
        ...
        >
          <div className="text-center space-y-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Built for Perfection</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-left">
            {[
              ...
              { icon: Sparkles, title: "✨ Bullet Point Enhancer", desc: "Transform passive items into quantified accomplishments.", desktopOnly: true },
              ...
              return (
                <motion.div
                  ...
                  className={`p-5 rounded-xl bg-white/[0.01] border border-white/5 space-y-3 hover:border-slate-800 transition-all duration-300 ${feat.desktopOnly ? 'hidden sm:block' : ''}`}
                >
        ...
        >
          <div className="text-center space-y-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Aesthetic layouts</span>
            ...
            {templatesList.map((t) => (
              <button
                ...
                className={`px-3 py-1 rounded-md text-[9px] sm:text-xs font-bold uppercase transition-all ${
                  activeTemplate === t ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:text-white'
                }`}
              >
                {t}
              </button>
          ...
          <motion.div
            variants={fadeInUpVariant}
            className="max-w-xl mx-auto rounded-xl bg-[#0c0c10] border border-white/5 shadow-lg overflow-hidden p-4 sm:p-6 text-left space-y-4 hover:border-slate-800 transition-all duration-300"
          >
            <div className="flex justify-between items-center border-b border-white/5 pb-2">
              ...
              <span className="text-[9px] sm:text-xs font-bold text-slate-500 uppercase tracking-widest">{activeTemplate} Layout</span>
            </div>
              ...
              <div className="space-y-2 font-sans hidden sm:block">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['React', 'TypeScript', 'Node.js', 'PostgreSQL', 'Docker', 'AWS'].map((sk) => (
                    <span key={sk} className="px-2 py-0.5 bg-white/5 rounded border border-white/10 text-[8px] sm:text-[9px] font-medium text-slate-200">{sk}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        >
          <div className="text-center space-y-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Intelligent Engine</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto text-left">
            {[
              ...
              <motion.div
                key={idx}
                variants={fadeInUpVariant}
                className="p-5 rounded-xl bg-white/[0.01] border border-white/5 hover:border-slate-800 transition-all duration-300"
              >
                <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-2 text-slate-300">{cap.title}</h4>
        ...
        <motion.section
          ...
          className="p-5 sm:p-8 rounded-xl bg-white/[0.01] border border-white/5 text-left max-w-4xl mx-auto space-y-4"
        >
          <div className="space-y-1">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Auditing System</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-xs font-light">
            {[
              ...
              <div key={idx} className={`space-y-1 border-l-2 border-slate-700 pl-3 py-0.5 ${item.desktopOnly ? 'hidden sm:block' : ''}`}>
        ...
        >
          <div className="text-center space-y-1">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Alignment Index</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3 max-w-5xl mx-auto text-left">
            {[
              ...
              <motion.div
                key={idx}
                variants={fadeInUpVariant}
                className={`p-4 rounded-lg bg-white/[0.01] border border-white/5 flex flex-col justify-between hover:border-slate-800 transition-all duration-300 ${gap.desktopOnly ? 'hidden sm:flex' : ''}`}
              >
                <div>
                  <span className="text-[9px] sm:text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none">{gap.title}</span>
                  <p className="text-base sm:text-lg font-bold text-white mt-1">{gap.val}</p>
                </div>
        ...
        >
          <div className="text-center space-y-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Why Choose Us</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-left">
            {[
              ...
              <motion.div
                key={idx}
                variants={fadeInUpVariant}
                className="p-5 rounded-xl bg-white/[0.01] border border-white/5"
              >
                <Check className="w-4 h-4 text-slate-300 mb-2" />
        ...
        >
          <div className="text-center space-y-1">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">User success</span>
          ...
          <div className="hidden sm:flex flex-wrap justify-center gap-1 max-w-md mx-auto bg-white/[0.02] p-1 rounded-lg">
            {testimonialTabs.map((tab) => (
              <button
                ...
                className={`px-3 py-1 rounded-md text-[9px] sm:text-xs font-bold uppercase transition-all ${
                  activeTestimonialTab === tab ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:text-white'
                }`}
              >
                {tab}
              </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-2xl mx-auto text-left">
            <AnimatePresence mode="popLayout">
              {filteredTestimonials.slice(0, 1).map((t) => (
                ...
                >
                  ...
                  <div className="flex items-center space-x-2.5">
                    <div className="w-7 h-7 rounded-full bg-slate-800 flex items-center justify-center font-bold text-[10px] text-white shrink-0">{t.avatar}</div>
                    <div>
                      <h4 className="text-[10px] sm:text-xs font-bold text-white">{t.name}</h4>
                      <p className="text-[9px] sm:text-[10px] text-slate-500">{t.category} • <span className="text-slate-400 font-medium">{t.company}</span></p>
                    </div>
                  </div>
                </div>
              ))}
              ...
              {filteredTestimonials.slice(1, 2).map((t) => (
                <div
                  key={t.name}
                  className="p-4 rounded-xl bg-white/[0.01] border border-white/5 space-y-3 hidden sm:block"
                >
                  <p className="text-[11px] sm:text-xs text-slate-300 italic leading-relaxed font-light">"{t.quote}"</p>
                  <div className="flex items-center space-x-2.5">
                    <div className="w-7 h-7 rounded-full bg-slate-800 flex items-center justify-center font-bold text-[10px] text-white shrink-0">{t.avatar}</div>
        ...
        <motion.section
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.4 }}
          className="p-5 sm:p-8 rounded-xl bg-white/[0.01] border border-white/5 text-left max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 items-center shadow-md"
        >
          <div className="space-y-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Recruiter perspective</span>
          ...
          <div className="p-4 bg-[#0a0a0c] rounded-lg border border-white/5 space-y-2.5 font-mono text-[10px] sm:text-xs text-slate-400 w-full overflow-x-auto">
            <p className="text-white font-bold text-xxs uppercase">Verification Checklist</p>
            <ul className="space-y-1.5">
              <li className="flex items-center space-x-2"><Check className="w-3.5 h-3.5 text-slate-300 shrink-0" /><span className="text-xxs">Scannable, clean experience item structure</span></li>
              <li className="flex items-center space-x-2"><Check className="w-3.5 h-3.5 text-slate-300 shrink-0" /><span className="text-xxs">No tables, custom canvas bars, or graphic stars</span></li>
              <li className="flex items-center space-x-2"><Check className="w-3.5 h-3.5 text-slate-300 shrink-0" /><span className="text-xxs">Standard, legible contact coordinate fields</span></li>
            </ul>
          </div>
        </motion.section>
        ...
        >
          <div className="text-center space-y-1">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Clarifications</span>
          ...
          <div className="space-y-2.5 text-left">
            {faqData.map((faq, idx) => (
              <motion.div
                key={idx}
                variants={fadeInUpVariant}
                className="p-3.5 rounded-lg bg-white/[0.01] border border-white/5 transition-all duration-300"
              >
                <button
                  onClick={() => setOpenFaq(openFaq === idx ? null : idx)}
                  className="w-full flex justify-between items-center py-1 text-xs font-bold text-white hover:text-slate-300 focus:outline-none"
                >
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap justify-center gap-1.5 max-w-lg mx-auto">
            {['Software Engineering', 'Data Analytics', 'Product Management', 'Finance & Ops', 'Product Design'].map((role) => (
              <span key={role} className="px-2.5 py-1 rounded-full bg-white/[0.02] border border-white/5 text-[9px] sm:text-xs text-slate-300 cursor-default whitespace-nowrap">{role}</span>
            ))}
          </div>
        </section>
        ...
        <motion.section
          ...
          className="p-6 sm:p-10 md:p-14 rounded-2xl bg-white/[0.01] border border-white/5 text-center max-w-3xl mx-auto space-y-6 relative overflow-hidden"
        >
          <div className="space-y-2">
          ...
          <button onClick={onGetStarted} className="px-5 py-2.5 sm:px-6 sm:py-3.5 bg-white hover:bg-slate-100 text-slate-950 font-bold text-xs rounded-lg transition-all uppercase tracking-wider inline-flex items-center space-x-1.5">
      ...
      <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50">
        <div className="w-14 h-14 sm:w-16 sm:h-16 bg-[#0d0d10] rounded-full border border-slate-800 shadow-lg relative overflow-hidden shrink-0 flex items-center justify-center">
          <AladdinBot />
        </div>
      </div>

      {/* SECTION 12: DETAILED PREMIUM FOOTER */}
      <footer className="w-full bg-[#050508] border-t border-white/5 py-10 sm:py-14 px-4 sm:px-8 relative z-10 text-[10px] sm:text-xs text-slate-500">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-5 gap-6 mb-8 text-left">
          ...
          <div className="col-span-2 space-y-3 pr-4">
            <div className="flex items-center space-x-2">
              <div className="p-1 bg-slate-800 rounded-md text-white shrink-0">
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1.5 font-medium">
              <li><a href="#" className="hover:text-slate-300 transition-colors">AI Builder</a></li>
              <li><a href="#" className="hover:text-slate-300 transition-colors">ATS Analyzer</a></li>
              <li><a href="#" className="hover:text-slate-300 transition-colors">JD Matcher</a></li>
            </ul>
          </div>
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1.5 font-medium font-light">
              ...
              <li><a href="#" className="hover:text-slate-300 transition-colors">Scan Rules</a></li>
            </ul>
          </div>
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1.5 font-medium font-light">
              <li><a href="#" className="hover:text-slate-300 transition-colors flex items-center gap-1"><Github className="w-3 h-3" /><span>GitHub</span></a></li>
              <li><a href="#" className="hover:text-slate-300 transition-colors">Support</a></li>
            </ul>
          </div>

        </div>

        <div className="max-w-7xl mx-auto pt-6 flex flex-col md:flex-row items-center justify-between gap-4 border-t border-white/5 font-medium text-center md:text-left">
