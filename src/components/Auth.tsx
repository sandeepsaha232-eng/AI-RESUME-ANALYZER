...
export default function Auth({ onSuccess, onCancel }: AuthProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!sessionToken) {
        // If the user registered successfully but no session token is returned (e.g. email verification is required),
        // we show a beautiful, user-friendly success message, clear any inputs, and switch to Login mode.
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col md:flex-row transition-colors duration-300 font-sans overflow-x-hidden">
      {/* Decorative Side Column on desktop */}
      <motion.div
        initial={{ opacity: 0, x: -60 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
        className="hidden md:flex md:w-5/12 bg-indigo-600 dark:bg-indigo-900 relative overflow-hidden flex-col justify-between p-12 text-white"
      >
      ...
      <motion.div
        ...
        className="flex-grow flex flex-col justify-between p-6 sm:p-12 md:w-7/12 max-w-xl mx-auto w-full"
      >
        ...
        <div className="my-auto py-8">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {showSignUpSuccess && (
                <div className="p-4 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/50 rounded-xl mb-6 text-sm text-emerald-700 dark:text-emerald-300">
                  Account created successfully! A confirmation email has been sent. Please check your inbox and verify your email to log in.
                </div>
              )}
